...
( function() {
	CKEDITOR.addCss( 'body.autogrow:after { content: \'\';margin:0;padding:0;border:0;clear:both;width:1px;height:1px;display:block; }' );
	...
	function initIframeAutogrow( editor ) {
		...
		// Actual content height, figured out by expanding editor's contents
		// by invisible clearing elements defined in :before and :after
		function contentHeight() {
			var styles = window.getComputedStyle( editor.editable().$ );

			var maxHeight = 0,
				editable = editor.editable(),
				all = editable.find( '*' ),
				lowestItem = all.getItem( 0 );

			editable.addClass( 'autogrow' );

			for ( var i = 0; i < all.count(); i++ ) {
				var item = all.getItem( i ).$;
				var height = item.offsetTop + item.offsetHeight;
			...
			var itemStyles = window.getComputedStyle( lowestItem );

			var bodyMarginBottom = parseInt( styles.marginBottom, 10 );
			var elemMarginBottom = parseInt( itemStyles.marginBottom, 10 );

			editable.removeClass( 'autogrow' );

			return ( bodyMarginBottom > elemMarginBottom ) ? maxHeight + bodyMarginBottom : maxHeight + elemMarginBottom;
		}
